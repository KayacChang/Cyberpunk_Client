...
export function create(app, reel) {
    ...
    const slot = Slot({
        view: scene,
        reelStrips: reel,
        textures: TextureManager(app, symbolConfig),
    });
    ...
    const grid = Grid(app, scene.getChildByName('grid'));
    ...
    async function showRandomWild({randomWild}) {
        ...
        randomWild.forEach((row, rowIndex) => {
            ...
            row.forEach(async (colIndex) => {
                const symbol = slot.reels[rowIndex].displaySymbols[colIndex];
    ...
    async function showMaybeBonus(reelIndex) {
        const targets = scene.getChildByName(`maybeBonus@${reelIndex}`);
...
function Counter(it) {
    //
    const numbers = it.children.filter(({name}) => !isNaN(name));
